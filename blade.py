...
class RBMtDBN ():
    ...
    def extract_features(self, test_val = None, rbm_usage = True):
        ...
        self.write_output(extracted_features, extracted_test_features = self.extracted_test_features, test_val = test_val, rbm_usage = rbm_usage)
    
    def write_output(self, extracted_features, extracted_test_features = None, test_val = None, rbm_usage = True):
        ...
        for k in range(self.n_labels):
            ...
            if self.task == LEARNING and rbm_usage:
                ...
                for i in range(self.instances):
                    ...
                    aux_index+=1
            elif self.task == PREDICTION and rbm_usage:
        ...
        for k in range(self.n_labels):
            if rbm_usage:
                write_df = pd.DataFrame(data=csv_data_test[k],    # values
                             ...
                             columns=out_columns)
                write_df.to_csv(output_dir+test_filepath+'_'+str(k)+'.csv',quoting=1)
        
    
                write_df = pd.DataFrame(data=csv_data_train[k],    # values
                             index=list(range(len(csv_data_train[k]))),    # 1st column as index
                             columns=out_columns)
                write_df.to_csv(output_dir+'bm_train_'+str(k)+'.csv',quoting=1)
            
            if test_val == TEST:
                by_label_dataset[k].data_frame.to_csv(output_dir+'bm_original_train_'+str(k)+'.csv',quoting=1)
                
        if test_val == TEST:
            self.test_dataset.data_frame.to_csv(output_dir+'bm_original_test.csv',quoting=1)
    
    def run_tdbn(self, exe,filepath, out_filepath = None,test_filepath = None, alt_verbose = False, n_parents = 1):
        ...
        if test_filepath != None:
            args = [exe+'.jar', '-i', filepath, '-f', test_filepath, '-w', out_filepath, '-p',str(n_parents), '-pm']
        else:
            args = [exe+'.jar', '-i', filepath, '-p',str(n_parents), '-pm']
    ...
    def erase_files(self, test_val):
        ...
        for label in range(self.n_labels):
            ...
            os.remove(output_dir+"bm_train_"+str(label)+".csv")
            os.remove(output_dir+"bm_val_"+str(label)+".csv")
            os.remove(output_dir+"probs_"+str(label)+".csv")
...
cd = '1' #k - CD-k
epochs = '100'
#filepath = 'real_data/ArabicDigits'
filepath = 'example_data/binomial_1_2'
#filepath = 'multiclass_10'
learning_rate = '0.05'
#weight_decay = '1e-2'
weight_decay = '1e-4'
test_set_ratio= '0.2'
...
input_args=['rbm_tdbn.py', task, filepath, 
...
if __name__ == "__main__":   
    ...
    file_name_char = filepath.rfind('/')
    file_name = filepath[file_name_char +1:]
    dateTimeObj = datetime.now()
    strtime = dateTimeObj.strftime('%Y%m%d_%H%M%S')
    test_dir = os.path.join('results', file_name)
    output_dir = os.path.join(test_dir, strtime)
    ...
    val_runs = parsed_args.validation_runs
    rbm_usage = not parsed_args.no_rbm
    extraction_runs = parsed_args.extraction_runs
    
    tdbn_parents = parsed_args.tdbn_parents
        ...
        if task == PREDICTION:
            if rbm_usage:
                for j in range(val_runs):
                    ...
                    for k in range(extraction_runs):
                        network.extract_features(VAL)
                        for i in range(network.n_labels):
                            network.run_tdbn(exe='tDBN_bin',filepath = output_dir+'bm_train_'+str(i)+'.csv', 
                                             out_filepath = output_dir+'probs_'+str(i)+'.csv', 
                                             test_filepath = output_dir+'bm_val_'+str(i)+'.csv',
                                             n_parents = tdbn_parents)
                            
                        val_acc[j] = val_acc[j] + network.get_pred_results(output_dir+'/probs_', VAL)
                        network.erase_files(VAL)
                print(val_acc)
                network.rbms = val_rbms[val_acc.argmax()]
                network.extract_features(TEST)
                for i in range(network.n_labels):
                    network.weight_analyzer(network.rbms[i], filepath = output_dir+'fig/weights_'+str(i)+'.pdf')
                    network.run_tdbn(exe='tDBN_bin',filepath = output_dir+'bm_train_'+str(i)+'.csv', 
                                     out_filepath = output_dir+'probs_'+str(i)+'.csv', 
                                     test_filepath = output_dir+'bm_test_'+str(i)+'.csv',
                                     n_parents = tdbn_parents)
                    
                    network.run_tdbn(exe='tDBN_cat',filepath = output_dir+'bm_original_train_'+str(i)+'.csv',
                                     out_filepath = output_dir+'original_probs_'+str(i)+'.csv', 
                                     test_filepath = output_dir+'bm_original_test.csv',
                                     n_parents = tdbn_parents)
            else:
                network.extract_features(TEST, rbm_usage = rbm_usage)
                for i in range(network.n_labels):
                    network.run_tdbn(exe='tDBN_cat',filepath = output_dir+'bm_original_train_'+str(i)+'.csv', 
                                     out_filepath = output_dir+'original_probs_'+str(i)+'.csv', 
                                     test_filepath = output_dir+'bm_original_test.csv', 
            ...
            network.run_tdbn(filepath = output_dir+'bm_train_'+str(i)+'.csv', n_parents = tdbn_parents)
            

        if task == PREDICTION:
            if rbm_usage:
                acc[n] = network.get_pred_results(output_dir+'probs_', TEST)
            original_acc[n] = network.get_pred_results(output_dir+'original_probs_', TEST)
